...
export function Pagination({ currentPage = 1, totalPages = 5, onPageChange}) {
  ...
  const buildPageUrl = (page) => {
    ...
    return `${urlParams.pathname}?${urlParams.searchParams.toString()}`;
  }
  ...
  return (
    <>
    {
      ...
      <nav className={styles.pagination}>
        <a className={styles.paginationLink} style={prevPageStyle} onClick={handlePrevPage} href={buildPageUrl(currentPage - 1)}>
        ...
        </a>
        {
          pages.map(page => {
            ...
            <a 
                key={page} 
                className={`${styles.paginationLink} ${page === currentPage ? styles.isActive : null}`} 
